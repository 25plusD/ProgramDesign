...
private:
    ...
    void show_result(int, string, clock_t, clock_t);         
...
void SATSolverCDCL::initialize(string filename) {
    //读入cnf文件
    char c;   
    string s; 
    string filename1 = R"(E:\检查文件\)" + filename + ".cnf";
    ifstream fis(filename1);
    char buf[100];
    while (true) {
        fis >> c;
        if (c != 'p') 
...
void SATSolverCDCL::show_result(int result_status, string filename, clock_t StartTime, clock_t EndTime) {
    string suffix = filename + ".res";
...
void SATSolverCDCL::solve(string filename) {
    clock_t StartTime, EndTime;
    StartTime = clock();
    int result_status = CDCL();
    EndTime = clock();
    show_result(result_status,filename, StartTime, EndTime);
}


int CDCL2solve(string filename) {
